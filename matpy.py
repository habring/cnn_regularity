        ...
        if ntype.split('-')[-1] == 'svd': #If string indicates svd
            ...
            if vdims:    
                ...
                try:
                    ...
                    print('vdims only one dimension')
                    self.rowdims = 1
...
# compute entropy estimate of a random vector X according to [A new class of random vector entropy estimators and its applications in testing statistical hypotheses, M. N. Goria , N. N. Leonenko , V. V. Mergel & P. L. Novi Inverardi]
# X is an array, s.t. X[i,:] is the entire sample of the i-th component of random vector X and X[:,i] is the ith sample point of the vector
# That is, first index corresponds to components of the random vector, second index to different sample points
def entropy(X,k):
    N = X.shape[1]
    assert k < N, "k needs to be smaller then sample size"

    kth_nearest_neihgbors = np.zeros(len(X))
    distances = np.zeros([N, N])
    for i in range(N):
        for j in range(i):
            distances[i,j] = l2nsq(X[:,i]-X[:,j])
            distances[j,i] = distances[i,j]

    for i in range(N):
        kth_nearest_neihgbors[i] = np.partition(distances[i,:], k)[k]

    geom_mean = gmean(kth_nearest_neihgbors)
...
def tv_denoise(**par_in):
    ...
    if len(data_in.u0.shape)>2:
        data_in.u0 = np.squeeze(data_in.u0)
    ...
    if par.noise:
        ...
        u0 = np.clip(u0+np.random.normal(scale=par.noise*rg,size=u0.shape),0,1) #Add noise
